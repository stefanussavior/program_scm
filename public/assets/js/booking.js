...
if (booked_seats) {
    ...
    booked_seats.forEach(seatNo => {
        ...
        seat.classList.add("notAvaliable");
    })
}
...
function selectSuggestion(evt) {
    if (evt.target.nodeName == "LI") {
        this.previousElementSibling.value = evt.traget.innerText;
        this.innerText = "";
    }
}
...
function showSuggestions() {
    ...
    let suggestions = busData.filter(({bus_no}) => {
        ...
        const bus_num = bus_no.replace(regex, `<span class="h1">${this.value.toUpperCase()}</span>`);
