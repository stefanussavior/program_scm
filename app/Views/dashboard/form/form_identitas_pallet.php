                ...
                <form action="/submit_form_identitas_pallet" method="post">
                            ...
                            <div>
                                ...
                                <select name="nomor_gr" id="nomor_gr" class="form-control">
                                    ...
                                    <?php foreach ($master_gr as $gr): ?>
                                        <option value="<?php echo $gr['nomor_gr'] ?>"><?php echo $gr['nomor_gr']; ?>
                                        </option>
                                    <?php endforeach ?>
                                </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" name="num_paletization" id="num_paletization" class="form-control"
                                readonly>
                        </div>
                    </div>

                    <div class="row">
                        <div class="col-md-12 mb-1" id="additional_forms_container">
                        </div>
                    </div>
...
<script>
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response.data, function (index, row) {
                        ...
                        var namaBarangField = '<div class="col-sm-4 mb-4"><label>Nama Barang ' + (index + 1) + ' : </label><input name="nama_barang[]" class="form-control" value="' + row.nama_barang + '" readonly></div>';
                        fieldHtml += namaBarangField;

                        var qtyDatang = '<div class="col-sm-4 mb-4"><label>Jumlah Qty Barang ' + (index + 1) + ' : </label><input name="qty_dtg[]"  class="form-control qty" value="' + row.qty_dtg + '" readonly></div>';
                        fieldHtml += qtyDatang;

                        var satuanBerat = '<div class="col-sm-4 mb-4"><label>Satuan Berat Barang ' + (index + 1) + ' : </label><input name="satuan_berat[]"  class="form-control qty" value="' + row.satuan_berat + '" readonly></div>';
    ...
    var existingKodePallets = $('.kode-palletization').map(function () {
        return $(this).val();
    }).get();
    ...
    for (var i = 1; i <= jumlahPallet; i++) {
        ...
        // Listen to changes in num_paletization
        $('#num_paletization').on('input', function () {
